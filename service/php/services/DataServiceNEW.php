...
class DataService {
    ...
    private function __construct() {
        ...
        $this->AdmissionApplication = $this->getCollection("AdmissionApplication");
        $this->Person = $this->getCollection("Person");
        $this->AdmissionApplicationElement = $this->getCollection("AdmissionApplicationElement");
        $this->PermissionGroup = $this->getCollection("PermissionGroup");
        $this->UserGroup = $this->getCollection("UserGroup");
        $this->FileReference = $this->getCollection("FileReference");
        $this->Charge = $this->getCollection("Charge");
        $this->JSONForm = $this->getCollection("JSONForm");
        $this->JSONFormTemplate = $this->getCollection("JSONFormTemplate");
    ...
    public function getCollection($collectionName) {
        $collection = $collection = $this->mongoDB->$collectionName;
        if($collection) {
    ...
    public function saveDocument($documentType, $document, $loadDocument = true) {
        ...
        try {
            ...
            error_log(json_encode($document, JSON_PRETTY_PRINT) .
    ...
    public function loadDocument($documentType, $documentId, $projectedFields = null) {
       // error_log("Loading document for id: " . $documentId, $projectedFields);
        ...
        try {
            ...
            throw new Exception("Error createing mongo id with input='" . json_encode($documentId));
        }
        if($projectedFields) {
    ...
    public function findOneDocument($documentType, $criteria) {
        $application = $this->$documentType->findOne($criteria);
        return $application;
    }
    ...
    public function findDocuments($documentType, $criteria, $fields = null) {
        if($fields) {
...
//        $childInfo = $this->loadPerson($application['ChildId'], $fields['Child']);
    ...
    public function countValidApplications() {
        ...
        //error_log("Finding apps: " . json_encode($query));
...
//        $applicationCursor = $this->AdmissionApplication->find($query, $limit);
    ...
    function loadUsers($start=null, $limit=null, $sort=null, $criteria=null) {
        $applications = $this->sortCursor("Person", $criteria, $sort, $limit, $start);
        return $applications;
    }
    ...
    public function extractAndPersistDataObjectsFromApplication($applicationData) {
        ...
        foreach($parents as $parent) {
            $parental = $this->saveSubDocumentAndSwapForId($parent, 'Parental', 'ParentId', 'Person');
        ...
        $child = $this->saveSubDocumentAndSwapForId($applicationData, 'Child', 'ChildId', 'Person');
    ...
    public function saveSubDocumentAndSwapForId(&$containingData, $dataMemberName, $dataMemberId, $documentType) {
        ...
        if(array_key_exists($dataMemberId, $containingData)) {
            ...
            $subData['_id'] = new MongoId($containingData[$dataMemberId]);
        }
        $subDocument = $this->saveDocument($documentType, $subData);
        ...
        foreach($applications as $application) {
            ...
            if(!$hasChildArray) {
                ...
                foreach($hasChildArray as $parental) {
                    //error_log("Looking at parental " . json_encode($parental, JSON_PRETTY_PRINT));
    ...
    public function loadUserByUsernameAndPassword($strUsername, $strPassword) {
        ...
        if(password_verify($strPassword, $storedCipher)) {
    ...
    public function findUserLogin() {
        ...
        error_log("Looking for user with session " . $sessionId);
     ...
     */
    function httpRequest($fields, $action, $url, $useAuthorizedUser = false, $useBinary = false)
    {
        ...
        if ($fields) {
            if (!array_key_exists('assignee', $fields)) {
                if ($useAuthorizedUser && $this->getUser()) {
                    $fields['assignee'] = $this->getUser()->system_id;
                } else if ($this->getActor()) {
                    $fields['assignee'] = $this->getActor()->system_id;
                }
            }
        }
